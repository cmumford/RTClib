...
class Decoder(srd.Decoder):
    ...
    def handle_reg_0x00(self, b):  # Seconds
        ...
        self.putd(6, 0, [Ann.BIT_SECONDS, ['Second: %d' % self.time.seconds, 'Secs: %d' % self.time.seconds, 'S']])
        self.putd(7, 0, [Ann.REG_SECONDS, ['Seconds', 'Secs', 'S']])

    def handle_reg_0x01(self, b):
        self.putr(7)
        self.time.minutes = bcd2int(b & 0x7f)
        self.putd(6, 0, [Ann.BIT_MINUTES, ['Minute: %d' % self.time.minutes, 'M: %d' % self.time.minutes, 'M']])
        self.putd(7, 0, [Ann.REG_MINUTES, ['Minutes', 'Mins', 'M']])

    def handle_reg_0x02(self, b):
        self.putr(7)
        self.time.hours = bcd2int(b & 0x1f)
        self.putd(4, 0, [Ann.BIT_HOURS, ['Hour: %d' % self.time.hours, 'H: %d' % self.time.hours, 'H']])
        twentyfour = 1 if bcd2int(b & 0b01000000) else 0
        PM = 1 if bcd2int(b & 0b00100000) else 0
        if twentyfour:
            self.putd(6, 6, [Ann.BIT_12_24_HOURS, ['12-hour mode', '12 hr. mode', '12']])
        else:
            self.putd(6, 6, [Ann.BIT_12_24_HOURS, ['24-hour mode', '24 hr. mode', '24']])
        if PM:
    ...
    def handle_reg_0x03(self, b):  # Day of week
        ...
        self.putd(2, 0, [Ann.BIT_DAY, ['Day of week: %d' % self.time.days, 'DOW:%d' % self.time.days, 'D']])
    ...
    def handle_reg_0x04(self, b):
        ...
        self.putd(5, 0, [Ann.BIT_DATE, ['Date: %d' % self.time.date, 'D: %d' % self.time.date, 'D']])
    ...
    def handle_reg_0x05(self, b):
        ...
        self.putd(4, 0, [Ann.BIT_MONTH, ['Month: %d' % self.time.months, 'M: %d' % self.time.months, 'M']])
        c = bcd2int(b & 0x80)
        self.putd(7, 7, [Ann.BIT_CENTURY, ['Century: %d' % c, 'C: %d' % c, 'C']])
        self.putd(7, 0, [Ann.REG_MONTH, ['Month', 'Mon.', 'M']])

    def handle_reg_0x06(self, b):
        self.time.years = bcd2int(b & 0xff)
        self.putd(7, 0, [Ann.BIT_YEAR, ['Year: %d' % self.time.years, 'Y: %d' % self.time.years, 'Y']])
        self.putd(7, 0, [Ann.REG_YEAR, ['Year', 'Yr.', 'Y']])

    def handle_reg_0x07(self, b):  # Alarm 1 seconds
        self.alarm1.m1 = bcd2int(b & 0b10000000) >> 7
        self.putd(7, 7, [Ann.BIT_AE, ['A1M1: %d' % self.alarm1.m1, 'A1: %d' % self.alarm1.m1, 'A1']])
        self.alarm1.seconds = bcd2int(b & 0b01111111)
        self.putd(6, 0, [Ann.BIT_SECONDS, ['Seconds: %d' % self.alarm1.seconds, 'S: %d' % self.alarm1.seconds, 'S']])
    ...
    def handle_reg_0x08(self, b):  # Alarm 1 minutes
        ...
        self.putd(7, 7, [Ann.BIT_AE, ['A1M2: %d' % self.alarm1.m2, 'A2: %d' % self.alarm1.m2, 'A2']])
        self.alarm1.minutes = bcd2int(b & 0b01111111)
        self.putd(6, 0, [Ann.BIT_MINUTES, ['Minutes: %d' % self.alarm1.minutes, 'MM: %d' % self.alarm1.minutes, 'M']])
    ...
    def handle_reg_0x09(self, b):  # Alarm 1 hours
        ...
        self.putd(7, 7, [Ann.BIT_AE, ['A1M3: %d' % self.alarm1.m3, 'A3: %d' % self.alarm1.m3, 'A3']])
        self.alarm1.hours = bcd2int(b & 0b01111111)
        self.putd(6, 0, [Ann.BIT_HOURS, ['Hours: %d' % self.alarm1.hours, 'HH: %d' % self.alarm1.hours, 'H']])
    ...
    def handle_reg_0x0a(self, b):  # Alarm 1 day/date
        ...
        self.putd(7, 7, [Ann.BIT_AE, ['A1M4: %d' % self.alarm1.m4, 'A1: %d' % self.alarm1.m4, 'A4']])
        if self.alarm1.dy_dt:
            self.putd(6, 6, [Ann.BIT_DY_DT, ['Day', 'Day', 'D']])
            self.putd(5, 0, [Ann.BIT_DAY, ['Day of week: %d' % date, 'DOW: %d' % date, 'D']])
            self.alarm1.dow = date
        else:
            self.putd(6, 6, [Ann.BIT_DY_DT, ['Date', 'Date', 'D']])
            self.putd(5, 0, [Ann.BIT_DATE, ['Day of month: %d' % date, 'DOM: %d' % date, 'D']])
    ...
    def handle_reg_0x0b(self, b):  # Alarm 2 minutes
        ...
        self.putd(7, 7, [Ann.BIT_AE, ['A2M2: %d' % self.alarm2.m2, 'A2: %d' % self.alarm2.m2, 'A2']])
        self.alarm2.minutes = bcd2int(b & 0b01111111)
        self.putd(6, 0, [Ann.BIT_MINUTES, ['Minutes: %d' % self.alarm2.minutes, 'Min: %d' % self.alarm2.minutes, 'M']])
    ...
    def handle_reg_0x0c(self, b):  # Alarm 2 hours
        ...
        self.putd(7, 7, [Ann.BIT_AE, ['A2M3: %d' % self.alarm2.m3, 'A3: %d' % self.alarm2.m3, 'A3']])
        t4 = 1 if bcd2int(b & 0b01000000) else 0
        am = 1 if bcd2int(b & 0b00100000) else 0
        self.alarm2.hours = bcd2int(b & 0b00011111)
        self.putd(6, 6, [Ann.BIT_12_24_HOURS, ['12/24: %d' % t4, '24: %d' % t4, '24']])
        self.putd(5, 5, [Ann.BIT_AM_PM, ['AM/PM: %d' % am, 'AM: %d' % am, 'AM']])
        self.putd(4, 0, [Ann.BIT_HOURS, ['Hour: %d' % self.alarm2.hours, 'H: %d' % self.alarm2.hours, 'H']])
    ...
    def handle_reg_0x0d(self, b):  # Alarm 2 Day/Date
        ...
        self.putd(7, 7, [Ann.BIT_AE, ['A2M4: %d' % self.alarm2.m4, 'A4: %d' % self.alarm2.m4, 'A4']])
    ...
    def handle_reg_0x0e(self, b):  # Control register
        ...
        self.putd(0, 0, [Ann.BIT_ALM1, ['Alarm 1: %d' % alarm1, 'ALM1: %d' % alarm1, 'A']])
        self.putd(1, 1, [Ann.BIT_ALM2, ['Alarm 2: %d' % alarm2, 'ALM2: %d' % alarm2, 'A']])
        self.putd(2, 2, [Ann.BIT_INTCN, ['Interrupt control: %d (%s)' % (intcn, intcnmsg), 'INTCN: %d' % intcn, 'I']])
        self.putd(4, 3, [Ann.BIT_RS, ['RS: %s' % rsn, 'RS: %d' % rs, 'R']])
        self.putd(5, 5, [Ann.BIT_CONV, ['CONV: %d' % conv, 'CONV: %d' % conv, 'C']])
        self.putd(6, 6, [Ann.BIT_BBSQW, ['BBSQW: %d' % bbsqw, 'BBSQ: %d' % bbsqw, 'S']])
        self.putd(7, 7, [Ann.BIT_EOSC, ['Enable oscillator: %d (%s)' % (eosc, eoscmsg), 'EOSC: %d' % eosc, 'E']])
    ...
    def handle_reg_0x0f(self, b):  # Status
        ...
        self.putd(0, 0, [Ann.BIT_A1F, ['Alarm flag 1: %d' % a1f, 'AF1: %d' % a1f, 'A1']])
        self.putd(1, 1, [Ann.BIT_A2F, ['Alarm flag 2: %d' % a2f, 'AF2: %d' % a2f, 'A2']])
        self.putd(2, 2, [Ann.BIT_BUSY, ['Busy: %d' % BSY, 'AF2: %d' % BSY, 'B']])
        self.putd(3, 3, [Ann.BIT_EN32K, ['32K: %d' % EN32kHz, '32: %d' % EN32kHz, '3']])
        self.putd(7, 7, [Ann.BIT_OSF, ['OSF: %d (%s)' % (osf, osfmsg), 'OSF: %d' % osf, 'O']])
        self.putd(7, 0, [Ann.REG_STATUS, ['Status', 'St', 'S']])
        self.status = Status()

    def handle_reg_0x10(self, b): # Aging offset.
        temp = b
        self.putd(7, 0, [Ann.BIT_AOF, ['Aging offset: %d' % temp, 'Aging: %d' % temp, 'A']])
        self.putd(7, 0, [Ann.REG_AOF, ['Aging offset', 'AOF', 'A']])

    def handle_reg_0x11(self, b):
        temp = b
        self.putd(7, 0, [Ann.BIT_TEMP_MSB, ['Tmsb: %d °C' % temp, 'Tm: %d' % temp, 'T']])
    ...
    def handle_reg_0x12(self, b):
        ...
        self.putr(1)
        self.putr(0)
        self.putd(7, 6, [Ann.BIT_TEMP_LSB, ['Tlsb: %.2f °C' % temp, 'Tl: %.2f' % temp, 'T']])
